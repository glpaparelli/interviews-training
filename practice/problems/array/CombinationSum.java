...
/*
 * > PROBLEM 39 (medium): Combination Sum
 *   Given an array of distinct integers "candidates" and a target 
 *   integer "target", return a list of all unique combinations of
 *   candidates where the choosen numbers sum to target. 
 ...
 * 
 *  > SOLUTION: 
 *    We use the recursion to build a conceptual tree where as root we 
 *    have the pair <array, target>. The first child will be the pair
 *    <array, target - a[0]>, the second child will be 
 *    <array starting from a[1], tarhet - a[1]>, and so on. 
 ...
 *        - the i+1-th brother start the array from the postion i+1
 *    To use this trick we have to first sort the array, so that we do
 *    not loose any element.
 * 
 *    The time complexity is O(N^(M/T)+1) where N is the number of candidates,
 *    M is the smallest candidate among all the given integers, and T is the
 *    target. Time is exponential but this is expected since the algorithm is
